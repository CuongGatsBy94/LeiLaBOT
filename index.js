 ...
 * @Last Modified time: 2025-10-22 23:08:58
 */
...
async function checkBirthdays() {/**
   ...
   function createScheduleEmbed(type, customDescription = null) {
       ...
       * @Author: CuongGatsBy94
       * @Date: 2025-10-05 04:12:42
       * @Last Modified by: CuongGatsBy94
       * @Last Modified time: 2025-10-22 22:22:42
       */
      ...
      // Hệ thống màu sắc
      // Đã khai báo 'colors' ở nơi khác, nên sử dụng var thay vì const để tránh lỗi redeclaration
      var colors = {
          primary: 0x5865F2,    // Discord Blurple
          success: 0x57F287,    // Discord Green
          warning: 0xFEE75C,    // Discord Yellow
          ...
          try {
              ...
              if (command === 'help' || command === 'commands') {
                  const embed = createEmbed('primary', '🤖 LeiLaBOT - Hệ thống lệnh', 
                      ...
                      .addFields(
                          ...
                          {
                              name: '⏰ Tự động',
                              value: '```setschedule, testschedule, testschedulenow, testallschedules, setbirthday```',
                              inline: true
                          },
                          {
                              name: '🎮 Giải trí',
              ...
              if (command === 'setwelcome') {
                  ...
                  const embed = createEmbed('success', '✅ Đã thiết lập kênh chào mừng', 
                      `Đã đặt kênh chào mừng thành: ${channel.toString()}\n\n**Tin nhắn mẫu:**\n${welcomeMessage.replace('{user}', message.author.toString()).replace('{server}', message.guild.name)}`);
                  await message.reply({ embeds: [embed] });
              }
              ...
              if (command === 'channelinfo') {
                  ...
                  const embed = createEmbed('info', '📊 Thông tin cấu hình channels', 
                      'Dưới đây là thông tin các kênh đã được cấu hình:')
                      .addFields(
                          { 
                              name: '👋 Kênh chào mừng', 
                              value: welcomeConfig.welcomeChannel ? `<#${welcomeConfig.welcomeChannel}>` : '`Chưa thiết lập`', 
                              inline: true 
                          },
                          { 
                              name: '😢 Kênh tạm biệt', 
                              value: welcomeConfig.goodbyeChannel ? `<#${welcomeConfig.goodbyeChannel}>` : '`Chưa thiết lập`', 
                              inline: true 
                          },
                          { 
                              name: '🎭 Role chào mừng', 
                              value: welcomeConfig.welcomeRole ? `<@&${welcomeConfig.welcomeRole}>` : '`Chưa thiết lập`', 
                              inline: true 
                          },
                          { 
                              name: '⏰ Kênh tin nhắn tự động', 
                              value: botConfig.scheduleChannel ? `<#${botConfig.scheduleChannel}>` : '`Chưa thiết lập`', 
                              inline: true 
                          },
                          { 
                              name: '📨 Kênh log DM', 
                              value: botConfig.dmLogChannel ? `<#${botConfig.dmLogChannel}>` : '`Chưa thiết lập`', 
                              inline: true 
                          }
                      )
                      .addFields(
                          { 
                              ...
                              value: welcomeConfig.welcomeMessage ? `\`${welcomeConfig.welcomeMessage}\`` : '`Chưa thiết lập`', 
                              inline: false 
                          },
                          { 
                              name: '👋 Tin nhắn tạm biệt', 
                              value: welcomeConfig.goodbyeMessage ? `\`${welcomeConfig.goodbyeMessage}\`` : '`Chưa thiết lập`', 
                              inline: false 
                          }
                      );
   ...
   client.on('messageCreate', async (message) => {
       ...
       try {
           // ==================== THÊM LỆNH TESTSCHEDULENOW VÀ TESTSEND VÀO ĐÂY ====================
           if (command === 'testschedulenow' || command === 'testsend') {
               if (!message.member.permissions.has('Administrator')) {
                   const embed = createEmbed('error', '❌ Thiếu quyền', 
                       'Bạn cần quyền **Administrator** để sử dụng lệnh này!');
